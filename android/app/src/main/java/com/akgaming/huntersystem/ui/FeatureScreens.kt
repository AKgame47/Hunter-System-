...
@Composable fun RestrictionsScreen(vm:HunterViewModel)=FeaturePage("APP RESTRICTIONS"){SwitchRow("Workout focus lock",vm.focusLock,vm::setFocusLock);Text("This is a voluntary in-app soft lock. Android only permits hard app blocking for managed device-owner/profile-owner deployments.",color=Muted)}
@Composable fun PrivacyScreen(vm:HunterViewModel)=FeaturePage("PRIVACY & DATA"){Text("Camera frames are processed on-device and are not intentionally uploaded.");Text("Account and workout data sync to Supabase under row-level security.");Text("Local AI models remain in app-private storage.");SelectionContainer{Text("Export summary: {name:${vm.displayName}, level:${vm.hunter.level}, xp:${vm.hunter.xp}, rank:${vm.hunter.rank}, quests:${vm.hunter.completedQuests}}")};Text("Account deletion requires a verified server deletion endpoint before release.",color=Muted)}
@Composable fun SettingsScreen()=FeaturePage("SETTINGS"){Text("Theme: GYMORA dark");Text("Camera: optional");Text("AI model downloads: explicit action only");Text("Health data sharing: disabled by default")}
@Composable fun AboutScreen()=FeaturePage("ABOUT GYMORA"){Text("GYMORA 0.3.0",fontWeight=FontWeight.Bold);Text("TRAIN · DISCIPLINE · EVOLVE");Text("Fitness guidance is educational and is not medical advice.",color=Muted)}

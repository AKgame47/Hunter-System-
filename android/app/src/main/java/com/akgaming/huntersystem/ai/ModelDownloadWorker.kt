...
class ModelDownloadWorker(context: Context, params: WorkerParameters) : CoroutineWorker(context, params) {
    override suspend fun doWork() = withContext(Dispatchers.IO) {
        val model = ModelCatalog.byId(inputData.getString(KEY_MODEL_ID).orEmpty()) ?: return@withContext Result.failure(workDataOf("error" to "Unknown model"))
        ...
        if (available < model.bytes + 128L * 1024 * 1024) return@withContext Result.failure(workDataOf("error" to "Not enough free storage"))
        ...
        try {
            ...
            val connection = (initial.openConnection() as HttpURLConnection).apply {
                ...
                setRequestProperty("User-Agent", "HunterSystem-Android")
            }
            connection.connect()
            val finalUrl = connection.url
            val allowedHosts = setOf("huggingface.co", "cdn-lfs.huggingface.co", "cas-bridge.xethub.hf.co")
            require(finalUrl.protocol == "https" && finalUrl.host in allowedHosts) { "Download redirected to an unapproved host" }
            ...
            require(digest.equals(model.sha256, ignoreCase = true)) { "Model checksum verification failed" }

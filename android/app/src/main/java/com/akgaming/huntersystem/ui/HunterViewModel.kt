...
class HunterViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun restoreSession(){viewModelScope.launch{authState=if(auth.hasSession())AuthState.AUTHENTICATED else AuthState.SIGNED_OUT;if(authState==AuthState.AUTHENTICATED)refreshProfile()}}
    fun signIn(email:String,password:String)=authAction{require(email.isNotBlank()&&password.isNotBlank()){ "Email and password are required" };auth.signIn(email,password);authState=AuthState.AUTHENTICATED;refreshProfile()}
    fun signUp(email:String,password:String)=authAction{require(email.isNotBlank()){ "Email is required" };require(password.length>=8){ "Password must contain at least 8 characters" };auth.signUp(email,password);if(auth.hasSession()){authState=AuthState.AUTHENTICATED;refreshProfile()}else{authState=AuthState.CONFIRM_EMAIL;authMessage="Check your email and open the verification link."}}
    ...
    fun completeCurrent(){
        if(questComplete||completionBusy)return
        if(currentObjective<quest.objectives.lastIndex) currentObjective++ else viewModelScope.launch {
            completionBusy=true
            runCatching { profiles.queueWorkoutCompletion(quest.rewardXp) }
                .onSuccess { questComplete=true; authMessage="Workout saved. Server-authoritative reward sync scheduled." }
                .onFailure { authMessage=it.message?:"Unable to save workout" }
